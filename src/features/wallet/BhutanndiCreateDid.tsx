 ...
 * bhutanndi ndi-lift/mint-selected look instead of shadcn Card colors.
 */
    ...
    <>
      {option.disabled ? (
        <span
          ...
          style={{
            background: 'rgb(var(--bhutanndi-tint) / 0.06)',
            color: 'var(--bhutanndi-text-faint)',
          }}
        >
          Coming Soon
        </span>
      ...
      <div className="mb-1 flex items-center gap-2">
        <h3 className="font-display text-bhutanndi-strong text-[14.5px] font-semibold">
          {option.title}
        </h3>
        {showSelected ? (
          <span
            ...
            style={{
              background: 'var(--bhutanndi-mint-12)',
              color: 'var(--bhutanndi-accent)',
            }}
          >
            Selected
          </span>
      ...
      <p className="text-bhutanndi-muted text-[13px] leading-[1.5]">
        {option.desc}
      </p>
    </>
  )
...
interface BhutanndiCreateDidProps {
  step: number
  totalSteps: number
 ...
 * real createDid/generateDidWeb submit logic as the non-bhutanndi CreateDid.tsx.
 * Outer chrome (header, Stepper, protocol/format/DID-method pickers) gets
 * the full bhutanndi treatment; the Polygon/Ethereum/did:web method-specific
 ...
 * bhutanndi mint palette via the theme's standard shadcn token layer, and their
 * real balance-check / key-generation logic was left untouched.
 */
export function BhutanndiCreateDid({
  step,
  activeProtocols,
  protocolOptions,
  selectedProtocol,
  ...
  submitting,
  submitLabel,
}: BhutanndiCreateDidProps): React.JSX.Element {
  return (
    <div className="p-6">
      <div>
        <PageHeader
        ...
        <Panel className="mt-6">
          <div>
            <h2 className="font-display text-bhutanndi-strong text-[15px] font-semibold">
              Select Protocol
            </h2>
            <p className="text-bhutanndi-muted mt-1 text-[13px]">
            ...
            {selectedProtocol ? (
              <div className="border-bhutanndi-subtle mt-6 border-t pt-5">
                <p className="text-bhutanndi-strong mb-3 text-[13.5px] font-medium">
            ...
            {selectedOption ? (
              <div className="mt-6">
                <span className="font-bhutanndi-mono text-bhutanndi-muted mb-2 block text-[10px] tracking-[0.16em] uppercase">
                  Select DID Method *
                </span>
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  {selectedDid && didExample ? (
                    <div className="font-bhutanndi-mono text-bhutanndi-muted rounded-md px-1 text-[12.5px] whitespace-nowrap">
                      <span>e.g.</span>{' '}
                      <span className="text-bhutanndi-strong">
                        {didExample}
                      </span>
                    </div>
                  ) : null}
        ...
        {didWebHosting ? (
          <Panel className="mt-6">
            ...
            <p className="text-bhutanndi-muted mt-1 text-[13px] leading-[1.5]">
            ...
            <div className="mt-5 flex flex-col gap-4">
              <div>
                <p className="text-bhutanndi-strong mb-1 text-[13px] font-medium">
                  Required hosting URL
                </p>
                <div className="border-bhutanndi-grid bg-bhutanndi-raised text-bhutanndi-strong rounded-[10px] border px-3 py-2 font-mono text-[13px] break-all">
              ...
              <div>
                ...
                <div className="relative">
                  <pre className="border-bhutanndi-grid bg-bhutanndi-raised text-bhutanndi-strong max-h-60 overflow-auto rounded-[10px] border p-4 font-mono text-[11.5px]">
                  ...
                  <div className="absolute top-2 right-2 flex gap-1">
                    <button
                      ...
                      className="ndi-hairline-btn border-bhutanndi-grid inline-flex h-7 w-7 cursor-pointer items-center justify-center rounded-md border bg-[rgb(var(--bhutanndi-tint)/0.05)]"
                    >
              ...
              <div className="flex items-start gap-2 pt-1">
                ...
                <Label
                  ...
                  className="text-bhutanndi-body cursor-pointer text-[13px] leading-[1.5]"
                >
          ...
          <div className="mt-6 flex flex-col items-end gap-2">
            {showBackToEdit ? (
              <button
                ...
                className="ndi-plainlink text-bhutanndi-muted text-[13px]"
              >
                ← Back to edit
              </button>
            ) : null}
