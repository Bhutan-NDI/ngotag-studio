...
const CreateSchema = () => {
	...
	const validSameAttribute = (
		...
		const attributeError:
			| string
			| string[]
			| FormikErrors<IAttributes>[]
			| undefined = formikHandlers?.errors?.attribute;
		...
		const isErrorAttribute =
			attributeError &&
			attributeError.length > 0 &&
			attributeError[index] &&
			attributeError[index][field];
		const isTouchedAttribute =
			attributeTouched &&
			attributeTouched.length > 0 &&
			attributeTouched[index] &&
			attributeTouched[index][field];
		const isValueAttribute =
			attributeValue &&
			attributeValue.length > 0 &&
			attributeValue[index] &&
			attributeValue[index][field];

		if (!(isTouchedAttribute && isErrorAttribute) && isValueAttribute) {
			const isValid =
				attributeValue.filter((item: IAttributes, i: number) => {
					const itemAttribute = item[field]?.trim()?.toLocaleLowerCase();
					const enteredAttribute = attributeValue[index][field]
						?.trim()
						?.toLocaleLowerCase();
					if (
						itemAttribute &&
						enteredAttribute &&
						itemAttribute === enteredAttribute
					) {
						return {
							...item,
							index: i,
