 ...
 * mapped onto the ngotag icon set. Anything unmapped falls back to `dashboard`
...
interface NgotagSidebarProps {
  open?: boolean
  onClose?: () => void
}

/**
 * The signed-in nav rail, restyled to the ngotag glass/mint language. The
 ...
 */
export function NgotagSidebar({
  open = false,
  onClose,
}: NgotagSidebarProps): React.JSX.Element {
  ...
  // Ngotag-only: the reference places "Organizations" (a real, already-shipped
  ...
  return (
    <>
      ...
      <aside
        aria-label="Main"
        data-open={open ? '1' : '0'}
        className="border-ngotag-subtle ease-ngotag fixed top-16 left-0 z-[55] flex h-[calc(100dvh-4rem)] w-[248px] flex-col overflow-y-auto border-r bg-[var(--ngotag-chrome-fill-strong)] px-3 py-5 backdrop-blur-[20px] backdrop-saturate-[140%] transition-transform duration-[260ms] min-[901px]:translate-x-0 min-[901px]:bg-[var(--ngotag-chrome-fill)]"
        style={{
      ...
      >
        ...
        <div className="border-ngotag-subtle my-4 h-px border-t" />
