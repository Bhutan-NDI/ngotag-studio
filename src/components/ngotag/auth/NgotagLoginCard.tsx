...
import { FIELD_CLASS, LABEL_CLASS } from '@/components/ngotag/ui/formStyles'
...
interface NgotagLoginCardProps {
...
/**
 * The real sign-in card, restyled to the ngotag design language. All the
 ...
 */
export function NgotagLoginCard({
  signInForm,
  onSubmit,
  loading,
  showPassword,
  ...
  onForgotPassword,
  signUpUrl,
}: NgotagLoginCardProps): React.JSX.Element {
  return (
    <>
      ...
      {success ? (
        <p
          role="status"
          aria-live="polite"
          className="text-ngotag-accent border-ngotag-grid relative z-[4] mb-5 flex items-start gap-2 rounded-xl border bg-[var(--ngotag-mint-08)] px-3.5 py-3 text-[13px] leading-[1.5]"
        >
          <Icon
            name="check"
            size={14}
              ...
              <FormItem>
                <span className="flex items-baseline justify-between gap-3">
                  ...
                  <button
                    ...
                    className="ndi-plainlink text-ngotag-accent text-[12.5px] disabled:pointer-events-none disabled:opacity-60"
                  >
                    {forgetPasswordLoading
                      ? 'Send reset link . . .'
                      : 'Forgot password?'}
