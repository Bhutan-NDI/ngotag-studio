...
export function HairlineButton({
  ...
  return (
    <button
      type={type}
      className={`ndi-hairline-btn border-ngotag-grid font-display text-ngotag-body inline-flex h-12 cursor-pointer items-center justify-center gap-2.5 rounded-xl border bg-[rgb(var(--ngotag-tint)/0.03)] px-6 text-[14.5px] font-semibold ${
        block ? 'w-full' : ''
      } ${className}`.trim()}
      {...props}
    >
