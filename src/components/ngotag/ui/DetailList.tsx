      ...
      {items.map((item, i) => (
        <div
          ...
          className={`grid gap-1 py-3.5 min-[641px]:grid-cols-[200px_1fr] min-[641px]:gap-6 ${
            i > 0 ? 'border-ngotag-subtle border-t' : ''
          }`}
        >
          <dt className="text-ngotag-faint text-[13px] leading-[1.5]">
            {item.label}
          </dt>
          <dd
            className={`text-ngotag-body m-0 text-[14px] leading-[1.6] break-words ${
              item.mono ? 'font-ngotag-mono text-[12.5px]' : ''
            }`}
          >
            {item.value}
          </dd>
