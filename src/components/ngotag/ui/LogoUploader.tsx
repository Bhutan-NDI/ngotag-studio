...
interface NgotagLogoUploaderProps {
  logoPreview: string
 ...
 * markup is ngotag-styled; no new logic.
 */
export function NgotagLogoUploader({
  logoPreview,
  setLogoPreview,
  setFieldValue,
  imgError,
  setImgError,
  existingLogoUrl,
}: Readonly<NgotagLogoUploaderProps>): React.JSX.Element {
  ...
  return (
    <div className={FIELD_BLOCK_CLASS}>
      ...
      <div className="border-ngotag-grid bg-ngotag-raised flex items-center gap-4 rounded-[10px] border p-4">
        <div className="border-ngotag-grid relative h-16 w-16 flex-none overflow-hidden rounded-[10px] border">
          <Image
            src={previewSrc}
            alt="Logo preview"
            fill
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <input
            ...
            className="font-ngotag-body text-ngotag-muted file:text-ngotag-accent file:font-display text-[13px] file:mr-3 file:cursor-pointer file:rounded-[8px] file:border-0 file:bg-[var(--ngotag-mint-12)] file:px-3 file:py-1.5 file:text-[12.5px] file:font-medium"
          />
          {imgError ? (
            <p
