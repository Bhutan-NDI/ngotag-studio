        ...
        return (
          ...
          >
            <span
              ...
              className="font-bhutanndi-mono inline-flex h-7 w-7 flex-none items-center justify-center rounded-full border text-[11px]"
              style={{
                borderColor:
                  done || active
                    ? 'var(--bhutanndi-border-strong)'
                    : 'var(--bhutanndi-border-grid)',
                background:
                  done || active
                    ? 'var(--bhutanndi-mint-12)'
                    : 'rgb(var(--bhutanndi-tint) / 0.03)',
                color:
                  done || active
                    ? 'var(--bhutanndi-accent)'
                    : 'var(--bhutanndi-text-faint)',
              }}
            >
            ...
            <span
              className={`hidden truncate text-[13px] min-[641px]:inline ${
                active
                  ? 'text-bhutanndi-strong font-medium'
                  : 'text-bhutanndi-muted'
              }`}
            >
              {step.label}
            </span>
            ...
            {i < steps.length - 1 ? (
              <span
                ...
                style={{ background: 'var(--bhutanndi-border-subtle)' }}
              />
            ) : null}
          </li>
        )
