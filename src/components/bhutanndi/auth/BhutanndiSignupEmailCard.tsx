...
interface BhutanndiSignupEmailCardProps {
  email: string
  error?: string
 ...
 */
export function BhutanndiSignupEmailCard({
  email,
  error,
  onChange,
  onBlur,
  ...
  onDismissFailure,
  onUseDifferentEmail,
}: BhutanndiSignupEmailCardProps): React.JSX.Element {
  ...
  return (
    <>
      ...
      {emailSent ? (
        <div className="border-bhutanndi-grid relative z-[4] flex flex-col gap-3 rounded-xl border bg-[var(--bhutanndi-fill-sunk)] px-4 py-3.5">
          <p className="text-bhutanndi-strong m-0 text-[13.5px] font-medium">
            Check your inbox
          </p>
          <p className="text-bhutanndi-muted m-0 text-[12.5px] leading-[1.5]">
            We sent a verification link to{' '}
            <span className="text-bhutanndi-strong font-medium">{email}</span>.
            Open it to continue creating your account — this page reopens
            automatically when you return.
          </p>
          <p className="text-bhutanndi-faint m-0 text-[11.5px]">
          ...
          {!locked ? (
            <button
              ...
              className="ndi-plainlink text-bhutanndi-accent self-start text-[12.5px] font-medium"
            >
