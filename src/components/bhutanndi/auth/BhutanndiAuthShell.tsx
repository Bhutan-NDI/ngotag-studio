...
import { Eyebrow } from '@/components/bhutanndi/ui/Eyebrow'
...
interface BhutanndiAuthShellProps {
  children: ReactNode
 ...
 */
export function BhutanndiAuthShell({
  children,
  title,
  lead,
  scene,
}: BhutanndiAuthShellProps): React.JSX.Element {
  return (
    <div className="relative flex min-h-dvh flex-col bg-[var(--bhutanndi-surface-canvas)]">
      <div
        aria-hidden="true"
      ...
      <main className="relative z-[1] mx-auto flex w-full max-w-[1200px] flex-1 items-center px-5 py-14 min-[641px]:px-8 min-[901px]:py-20">
        <div className="grid w-full items-center gap-12 min-[901px]:grid-cols-[1.05fr_1fr] min-[901px]:gap-20">
          ...
          <section className="hidden min-[901px]:block">
            ...
            <h1 className="text-bhutanndi-strong font-display mt-4 max-w-[460px] text-[clamp(30px,3.4vw,42px)] leading-[1.08] font-semibold tracking-[-0.03em] [text-wrap:balance]">
              {title}
            </h1>
            <p className="text-bhutanndi-muted mt-5 max-w-[440px] text-[17px] leading-[1.62] [text-wrap:pretty]">
              {lead}
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-2.5">
              {['Verified credentials', 'End-to-end encrypted'].map((label) => (
                <span
                  key={label}
                  className="border-bhutanndi-grid text-bhutanndi-accent font-bhutanndi-mono inline-flex items-center gap-2 rounded-full border bg-[var(--bhutanndi-fill-sunk)] px-3 py-2 text-[10px] tracking-[0.14em] uppercase"
                >
          ...
          <section className="w-full justify-self-center min-[901px]:justify-self-end">
            <div
              ...
              className="border-bhutanndi-grid relative mx-auto w-full max-w-[440px] rounded-[16px] border p-5 min-[561px]:p-7 min-[901px]:rounded-[20px] min-[901px]:p-8"
            >
              {children}
            </div>
          </section>
