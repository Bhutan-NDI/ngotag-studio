...
interface BhutanndiUserInfoCardProps {
  email: string
 ...
 */
export function BhutanndiUserInfoCard({
  email,
  values,
  errors,
  touched,
  ...
  onChangeEmail,
  loading,
}: BhutanndiUserInfoCardProps): React.JSX.Element {
  return (
    <>
      {success ? (
        <AuthAlert
      ...
      <span className="border-bhutanndi-grid text-bhutanndi-body relative z-[4] inline-flex items-center gap-2.5 self-center rounded-full border bg-[var(--bhutanndi-mint-04)] px-4 py-2 text-[13px]">
        <CheckCircle2 className="text-bhutanndi-accent h-3.5 w-3.5" />
        {email}
        <button
          type="button"
          onClick={onChangeEmail}
          className="ndi-plainlink text-bhutanndi-accent text-[12.5px]"
        >
          Change
        </button>
      </span>
