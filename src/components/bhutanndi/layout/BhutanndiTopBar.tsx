...
// real desktop sidebar's <SidebarProvider>. The bhutanndi top bar has no
...
interface BhutanndiTopBarProps {
  navOpen: boolean
  onToggleNav: () => void
}

/**
 * The fixed signed-in bar, restyled to the bhutanndi glass chrome. The
 ...
 */
export function BhutanndiTopBar({
  navOpen,
  onToggleNav,
}: BhutanndiTopBarProps): React.JSX.Element {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[60] h-16 bg-[var(--bhutanndi-chrome-fill)] backdrop-blur-[20px] backdrop-saturate-[140%]">
      <div className="border-bhutanndi-subtle pointer-events-none absolute inset-x-0 bottom-0 h-px border-t min-[901px]:left-[248px]" />
      <div className="border-bhutanndi-subtle pointer-events-none absolute inset-y-0 left-[247px] hidden w-px border-l min-[901px]:block" />
